...
Three independent agent attempts at one Git-deployment task all failed to deliver a password-authenticated endpoint.
% 同一 Git 部署任务的三次独立 agent 尝试都未能交付密码认证端点。
...
The profile answers in one view (Figure~\ref{fig:flamegraph}): all three runs share a \texttt{diagnose authentication} responsibility that consumes 46\% of their combined token budget (2.1M tokens) but only 21\% of operation count.
% profile 用一个视图回答（Figure~\ref{fig:flamegraph}）：三次执行共享一个 \texttt{diagnose authentication} 责任，消耗 46\% 的 token 预算（210 万 token）但只占 21\% 的 operation 数。
The same hierarchy replays under elapsed time: the subtree holds 37\% of wall-clock seconds.
% 同一层次按 elapsed time 重放：该子树占 37\% 的挂钟时间。
...
As a control, we replay the same 489 operations under native call trees and coarse action grouping.
% 作为对照，我们在 native call tree 与 coarse action 分组下重放相同的 489 个 operation。
Neither surfaces the pattern: the authentication work scatters across 105 native calls and six action kinds, with 102 of 105 labeled merely \texttt{run}.
% 两者都未呈现该模式：认证工作散落于 105 个 native call 和六个 action kind，其中 102 个仅标为 \texttt{run}。
Only semantic organization reunites the cross-run responsibility into one focusable path.

\paragraph{Use case 2: where did this project's agent budget go?}
% 用例 2：这个项目的 agent 预算去哪了？
A developer spent weeks building \sys{} and this paper with coding agents, accumulating 42 sessions (18 Codex, 24 Claude Code), 1{,}252 prompts, 5{,}620 LLM calls, and 1.38 billion tokens.
% 一位开发者花数周用 coding agent 构建 \sys{} 与本文，累积 42 个 session（18 Codex、24 Claude Code）、1,252 个 prompt、5,620 次 LLM 调用和 13.8 亿 token。
...
The same hierarchy replays under file-write width (Figure~\ref{fig:selfprofile-write}), with each created file appearing as a drilldown leaf.
% 同一层次按 file-write 宽度重放（Figure~\ref{fig:selfprofile-write}），每个被创建的文件作为下钻叶出现。
...
\end{figure*}
\emph{Takeaway.} One hierarchy attributes multiple resources with exact conservation; switching measures reveals bottlenecks that operation count alone would hide.
% 要点。同一层次精确守恒地归因多种资源；切换度量暴露出仅靠 operation 数会掩盖的瓶颈。
