...
(Table~\ref{tab:rq2-localization}; paired semantic-minus-raw intervals
[-.0003, .0029], [-.0116, .0103], and [-.0247, .0280]), confirming that
grouping plus retained evidence---not naming alone---drives the localization
gain over Direct-only.
% 语义与 \sys-Raw 两种细化达到相同的排序质量（Table~\ref{tab:rq2-localization}；配对的语义减 raw 区间为 [-.0003,.0029]、[-.0116,.0103] 与 [-.0247,.0280]），确认了"分组+保留的证据"（而非单独的命名）驱动相对 Direct-only 的定位增益。
