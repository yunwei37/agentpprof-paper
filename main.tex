...
information~\cite{bouma2009npmi} and calibrates an unsupervised cutoff with
one-dimensional $k$-means ($k=2$)~\cite{macqueen1967}; it reads only action sequences, never target annotations or resource weights.
% 统计 segmenter 用归一化点互信息 $\mathrm{NPMI}(a,b)=\ln[p(a,b)/(p_L(a)p_R(b))]/-\ln p(a,b)$ 为相邻 action 转移评分（所有概率取自同一转移样本空间），并用按出现次数加权的一维 $k$-means（$k=2$）校准无标签阈值——从最小与最大分数初始化、并列归入较低中心、以收敛中点为全局阈值，另在 action-changing 转移上拟合第二个阈值；detail 连续性只能删除、不能新增粗边界，detail 缺失时精确回退到粗决策；segmenter 只读取 session 身份、输入顺序、action 与可选 action detail，从不读取目标标注或资源权重。可选的 calibrated 模式在不重叠的参考分组上拟合单一标量阈值。
%% ===================================================================
\section{Evaluation}
% 评估
\label{sec:eval}
...
\emph{Direct-only} ranks by that diagnostic alone; \emph{Direct+\sys} breaks ties using the profiler's aggregated scores per semantic group;
\emph{Direct+\sys-Raw} is an ablation that replaces semantic interval names with raw action types (the grouping structure is identical; only the names differ); \emph{AgentProf-only} uses only the profiler without the benchmark diagnostic (Table~\ref{tab:rq2-localization}).
% Direct-only 仅按该诊断排序；Direct+\sys 用 profiler 的组分数打破并列；Direct+\sys-Raw 是将语义区间名替换为 raw action 类型的消融（分组结构相同，仅名称不同）；AgentProf-only 仅用 profiler 不用 benchmark 诊断（Table~\ref{tab:rq2-localization}）。
...
labeled queries). \sys-Raw ablates only \sys's semantic prefix; bold
...
\caption{Paired MAP differences with 95\% intervals (10,000 resamples). Left: profile improves all three benchmarks (intervals above zero). Right: semantic vs raw-action ablation indistinguishable (intervals span zero).}
...
\emph{Results.} Direct+\sys beats Direct-only by 0.031 [0.024, 0.039],
...
% 结果。Direct+\sys 相对 Direct-only 分别提高 .031 [.024,.039]、.107 [.093,.120] 与 .117 [.088,.148]（在 benchmark 定义的分层内对轨迹簇做 10,000 次配对重抽样），因此每个配对区间都位于零以上。
Grouping with retained evidence drives the ranking gain (Table~\ref{tab:rq2-localization}); semantic naming adds a distinct measured benefit: it concentrates the reader's attention, reducing evidence opened from 65\% to 53\% at equal quality (measured below). The semantic prefix also enables cross-run attribution (RQ1).
% 分组加保留证据驱动排序增益（Table~\ref{tab:rq2-localization}）；语义命名带来另一可测收益：它集中读者注意力，在同等质量下将打开的证据从 65\% 降至 53\%（下文测量）。语义前缀还支持跨运行归因（RQ1）。
...
The semantic skeleton reaches MAP .455 while opening only 53.0\% of the source content; the \sys-Raw skeleton requires 65.0\% to reach comparable quality (MAP .465).
...
A full end-to-end segmentation pass on 440 trajectories completes in 58.7 minutes, consuming 12{,}039{,}417 input and 312{,}433 output tokens, or 27{,}362 input and 710 output tokens per trajectory.
