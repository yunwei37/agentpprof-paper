...
The segmentation contract is implementation-neutral: an agent, a language model, or a statistical rule can all produce the same marks.
...
A control experiment replays the same operations and weights under three organizations (native call tree, coarse action grouping, and semantic hierarchy): the responsibility scatters across 105 native calls and six coarse action kinds (largest branch 39.42\% of its tokens), but only the semantic organization reunites it into one focusable cross-run path.
% 对照实验在三种组织（native call tree、coarse action 分组、语义层次）下重放相同的 operation 与权重：该责任在 native 中散落于 105 个 call、在 coarse 中跨六个 action kind（最大分支仅含其 token 的 39.42\%），只有语义组织将其重聚为一条可聚焦的跨运行路径。
...
The backend works in an annotation workspace where the CLI validates that marks are nested, noncrossing, and cover every source node (each source node corresponds to one turn in the trajectory), then emits one standard pprof protobuf profile that existing tools such as \texttt{go tool pprof} read directly.
% backend 在标注 workspace 中工作，CLI 校验标记嵌套、不交叉并覆盖每个 source node（每个 source node 对应轨迹中的一个 turn），然后输出一个标准 pprof protobuf profile，供 \texttt{go tool pprof} 等现有工具直接读取。
...
The backend reads the trajectory and writes interval annotations that map source positions to semantic tags and parent links; after validation \sys updates derived paths and the folded aggregate.
% backend 读取轨迹并写入区间标注，将 source 位置映射到语义标签与父链接；校验通过后 \sys 更新派生路径与折叠聚合。
Nonblocking hierarchy warnings flag degenerate structures (single-child recursion, flat fan-outs, coarse leaves), exposing annotation issues without forcing artificial depth or blocking generation.
% 非阻塞 hierarchy warning 标记退化结构（单子递归、flat fan-out、粗叶），暴露标注问题但不强制人为深度或阻塞生成。
...
The source population contains 41 real long-horizon agent trajectories, 3{,}146 source-native turns, and 5{,}750 operations, including three independent executions of one Git-deployment task whose 735 source nodes receive 96 recursive annotations.
% source population 含 41 条真实 long-horizon agent 轨迹、3,146 个 source-native turn 与 5,750 个 operation，其中包含同一 Git 部署任务的三次独立执行，其 735 个 source node 获得 96 个递归标注。
...
In this local-history scenario, the profiler initializes the annotation workspace directly from the raw session logs and materializes 10{,}423 source nodes: 42 sessions, 1{,}252 prompts, 5{,}620 LLM calls, and 3{,}509 tool calls carrying 1{,}380{,}863{,}014 provider-reported tokens.
% 在此本地历史场景中，profiler 直接从原始 session log 初始化标注 workspace，物化出 10,423 个 source node：42 个 session、1,252 个 prompt、5,620 个 LLM call 与 3,509 个 tool call，携带 1,380,863,014 个 provider-reported token。
...
\emph{Results.} Segmenting all 405 CodeTraceBench trajectories consumes 12{,}050{,}384 input and 231{,}886 output tokens in 37 minutes of active backend time.
% 结果。切分全部 405 条 CodeTraceBench 轨迹消耗 12,050,384 输入与 231,886 输出 token、37 分钟活跃 backend 时间。
