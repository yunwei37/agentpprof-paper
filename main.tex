...
A two-stage profile-guided variant first shows only the semantic operation skeleton, with no source content, and the agent selects at most five groups; stage two then opens only those groups' evidence.
...
Missed targets were absent from the agent's selection entirely, not truncated by the five-group limit.
% 遗漏的 target 完全不在 agent 的选择中，而非被五组上限截断。
