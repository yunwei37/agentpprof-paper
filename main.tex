...
% 我们比较七种分组策略（扁平摘要、固定会话分组、原生层次、原始操作分组、operation stack 分析、operation stack 加 oracle 字段，以及使用隐藏标注的完整 oracle），按组包含的隐藏正例比例对组排序。
...
% 我们运行留一数据集评估。
For each of 9 datasets, we train mapping rules on the other 8 and measure V-measure (a clustering-agreement score ranging from 0 to 1) between the mapping-derived \texttt{phase} field and the dataset's native \texttt{action} annotations, plus boundary F1 (precision and recall of phase-transition points) between mapped phase boundaries and native action boundaries.
% 对于 9 个数据集中的每一个，我们在其余 8 个上训练映射规则，测量映射衍生的 \texttt{phase} 字段与数据集原生 \texttt{action} 标注之间的 V-measure（聚类一致性分数，范围 0 到 1），以及映射阶段边界与原生操作边界之间的边界 F1（阶段转换点的精确率和召回率）。
...
% 作为上界对比，在真实阶段转换上训练的有监督边界预测器在 4/5 测试数据集上超过基于规则的映射（例如 OSWorld-Human 上 F1 约 0.77 vs 0.71），确认映射规则产生有用标签但仍有提升空间。
...
The slowest configurations process views with over 600 unique stacks, while 60 configurations that vary only the ranking criteria or stack depth (robustness checks) all complete within 1.6\,s.
% 最慢的配置处理超过 600 个唯一栈的视图，而 60 个仅改变排名标准或栈深度的配置（稳健性检查）全部在 1.6 秒内完成。
