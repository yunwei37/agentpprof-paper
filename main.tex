...
The algorithm computes $S$ by one recursive rule: $\textsc{Segment}(I)$ selects zero or more transition points inside interval $I$, which partition $I$ into consecutive child intervals; each child receives a short name starting with a verb name and is segmented recursively; selecting no transition terminates the branch.
...
granularity where recurrence lives, so the same short name starting with a verb name
...
reference groups.
% 统计 segmenter 用归一化点互信息 $\mathrm{NPMI}(a,b)=\ln[p(a,b)/(p_L(a)p_R(b))]/-\ln p(a,b)$ 为相邻 action 转移评分（所有概率取自同一转移样本空间），并用按出现次数加权的一维 $k$-means（$k=2$）校准无标签阈值——从最小与最大分数初始化、并列归入较低中心、以收敛中点为全局阈值，另在 action-changing 转移上拟合第二个阈值；detail 连续性只能删除、不能新增粗边界，detail 缺失时精确回退到粗决策；segmenter 只读取 session 身份、输入顺序、action 与可选 action detail，从不读取目标标注或资源权重。可选的 reference-calibrated 模式在不重叠的参考分组上拟合单一标量阈值。
%% ===================================================================
\section{Evaluation}
% 评估
\label{sec:eval}
...
\emph{Direct-only} ranks by that diagnostic alone; \emph{Direct+AgentProf} breaks ties by the profiler's group score;
...
0.816 B$^3$ F1, reference-calibrated recurrence reaches 0.734 / 0.801 using
...
harness, distinct from RQ2's 7{,}229-operation mixed-outcome population.}
\label{tab:rq4-cost}
\end{table}
...
A full end-to-end segmentation pass on 440 sessions (with no outcome labels visible to the segmenter) completes all 12 batches in 3{,}521.6\,s wall-clock time (58.7 minutes; summed worker time 6{,}661.7\,s) on two parallel workers, consuming 12{,}039{,}417 actual input tokens (10{,}929{,}408 reported cached, 90.8\%) and 312{,}433 output tokens---27{,}362 input and 710 output tokens per session---while one fresh pass over the three-session Git population takes 466.9\,s and 832{,}544 actual input tokens.
