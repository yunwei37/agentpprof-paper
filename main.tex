...
Codex~\cite{codex} reads each complete source-only packet once and emits sparse
...
requesting a depth.
% Codex 只读一遍完整的 source-only packet，并在 17,148 个 source-native 步骤上发出稀疏的完整路径 mark，物化之前不接触官方 stage 或 outcome：共 4,496 个 mark，语义深度为一层（3 个）、两层（2,873 个）、三层（1,588 个）与四层（32 个），且没有任何 prompt 指定深度。
...
Qwen3.6-27B one-shot & 0.174 & 1.000 & 0.296 & 0.000 \\
...
trajectories. Qwen3.6-27B one-shot uses the same prompt but degenerates to one segment per trajectory (boundary F1 = 0).}
\label{tab:rq3-codetrace}
\end{table}

\emph{Results.} Codex single-pass segmentation reaches 0.764 B$^3$ F1 and 0.480
boundary F1, beating the strongest automatic baseline by +0.101
[0.087, 0.116] and +0.214 [0.193, 0.235] (paired task-cluster
intervals).
The same prompt with Qwen3.6-27B degenerates to one segment per trajectory (boundary F1 = 0), showing that model capability rather than iterative refinement drives the result.
The marks conserve exactly 20{,}866 operations and
494{,}862{,}929 tokens.
% 结果。Codex 切分达到 0.764 B$^3$ F1 与 0.480 边界 F1，以 +0.101 [0.087,0.116] 与 +0.214 [0.193,0.235] 的配对任务簇区间超过最强自动基线。同一 prompt 用 Qwen3.6-27B 退化为每轨迹单个 segment（boundary F1 = 0），表明结果由模型能力而非迭代优化驱动。标记精确守恒 20,866 个 operation 与 494,862,929 个 token。
