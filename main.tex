...
task intent and workflow phase, not code paths.
% 智能体行为有根本不同。要回答上述问题，负责实体需要是任务意图和工作流阶段，而非代码路径。
...
\emph{Takeaway.} Segmentation is a one-time pass costing minutes of backend time per corpus (37 min for 405 trajectories); every later view, measure switch, and drilldown costs about one second.
% 要点。切分是一次性 pass，每语料需分钟级后端时间（405 条轨迹需 37 分钟）；之后每次视图、度量切换与下钻的成本约为一秒。
