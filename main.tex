...
The insight: token-heavy work happens at the conversation level, while repetitive tool calls concentrate deeper---a split that only multi-measure profiling reveals.
% 洞察：token 密集型工作发生在对话层，而重复的工具调用集中在更深层——这种分裂只有多度量 profiling 才能揭示。
...
Across 440 AgentRewardBench trajectories (7{,}229 operations, 51{,}904{,}621 tokens, both conserved exactly), ranking operations by count versus tokens yields high agreement: over the 77 of 125 tasks with at least three distinct operations, mean Kendall's tau-b is 0.886 [0.857, 0.915], Spearman's rho is 0.935 [0.917, 0.953], and pooled ranking agrees at tau-b 0.929; but 10 of 77 tasks fall below 0.7---stable in most cases, but the measure matters when it matters.
% 跨 440 条 AgentRewardBench 轨迹（7,229 个 operation、51,904,621 个 token，均精确守恒），按 count 与 token 排序的一致性高：在 125 个任务中至少有三个不同 operation 的 77 个上，平均 Kendall tau-b 为 0.886 [0.857, 0.915]，Spearman rho 为 0.935 [0.917, 0.953]，pooled 排序的 tau-b 为 0.929；但 77 个任务中有 10 个低于 0.7——大多数情形稳定，但关键时刻度量选择很重要。
...
first-stage selections never fell back to a default: the five-group budget
saturated on 99.5\% of queries, re-parsing the uncapped first-stage responses
...
operations.
% 在信息量等价的 \sys-Raw 骨架下，排序质量在统计上不变（MAP .465；配对差 +.010 [-.021,+.042]），但 reader 打开的内容显著更多：65.0\% 对 53.0\%，配对差 +.120 [+.103,+.137]，且多打开 2.80 [1.96,3.60] 个 evidence operation。
...
A full end-to-end segmentation pass on 440 trajectories (with no outcome labels visible to the segmenter) completes all 12 batches in 3{,}521.6\,s wall-clock time (58.7 minutes; summed worker time 6{,}661.7\,s) on two parallel workers, consuming 12{,}039{,}417 actual input tokens (10{,}929{,}408 reported cached, 90.8\%) and 312{,}433 output tokens---27{,}362 input and 710 output tokens per trajectory---while one fresh pass over the three-trajectory Git population takes 466.9\,s and 832{,}544 actual input tokens.
% 对 440 个 session 的完整端到端切分（切分器看不到 outcome 标签）在双并行 worker 上以 3,521.6 秒 wall-clock 时间（58.7 分钟；worker 时间合计 6,661.7 秒）完成全部 12 个 batch，消耗 12,039,417 个实际输入 token（其中报告缓存 10,929,408 个，占 90.8\%）与 312,433 个输出 token——每 session 27,362 输入与 710 输出 token；对三 session 的 Git population，一次全新完整 pass 需 466.9 秒与 832,544 个实际输入 token。
