...
% 一个算法驱动此模型：递归 operation 切分通读轨迹，在智能体责任变化处切开，为两侧区间命名并递归；动词引导的短名称为智能体提供了传统 profiling 从函数名免费获得的稳定标识符。
...
Levels need not come from annotation: fields already recorded literally (e.g., agent skill invocations) become stack levels at zero cost and compose with annotated semantic levels in one hierarchy.
% 层级不必来自标注：已被字面记录的字段（如 agent skill 调用）可以零成本成为栈层级，并与标注得到的语义层级组合在同一层次中。
...
information, $\mathrm{NPMI}(a,b)=\ln[p(a,b)/(p_L(a)p_R(b))]/-\ln p(a,b)$,
with all probabilities over one transition sample
space~\cite{bouma2009npmi}, and calibrates a unsupervised cutoff with
occurrence-weighted one-dimensional $k$-means
($k=2$)~\cite{macqueen1967}---initialized at the minimum and maximum scores,
ties to the lower center, converged midpoint as the global cutoff, with a
second cutoff fitted on action-changing transitions---treating detailed
recurrence as continuity that can remove but never add a coarse boundary,
with exact coarse fallback when detail is missing; the segmenter reads only
session identity, input order, action, and optional action detail, never
target annotations or resource weights.
An optional calibrated mode fits one scalar cutoff on disjoint
reference groups.
...
The population contains 41 real long-horizon agent trajectories, 3{,}146 source-native turns, and 5{,}750 operations, including three independent executions of one Git-deployment task whose 735 source nodes receive 96 recursive annotations.
% population 含 41 条真实 long-horizon agent 轨迹、3,146 个 source-native turn 与 5,750 个 operation，其中包含同一 Git 部署任务的三次独立执行——其 735 个 source node 获得 96 个递归标注。
...
At population scale, replaying the frozen 440-trajectory AgentRewardBench hierarchy (7{,}229 operations and 51{,}904{,}621 tokens, both conserved exactly) ranks the same operations once by count and once by tokens: over the 77 of 125 tasks with at least three distinct operations, mean per-task Kendall's tau-b~\cite{kendall1938} is 0.886 [0.857, 0.915] and Spearman's rho~\cite{spearman1904} is 0.935 [0.917, 0.953], the pooled ranking agrees at tau-b 0.929, and 10 of 77 tasks fall below 0.7---stable in the common case and changing decisions in a material minority.
% 在 population 尺度上，重放冻结的 440-session AgentRewardBench 层次（7,229 个 operation 与 51,904,621 个 token，均精确守恒），对相同 operation 分别按 count 与 token 排序：在 125 个任务中至少有三个不同 operation 的 77 个上，每任务平均 Kendall tau-b 为 0.886 [0.857,0.915]，Spearman rho 为 0.935 [0.917,0.953]，pooled 排序的 tau-b 为 0.929，77 个中有 10 个低于 0.7——常见情形稳定，少数关键情形改变决策。
...
A signed difference profile over the 338 bad--good pairs the tasks induce (bad minus good, Figure~\ref{fig:diff-flamegraph}) answers at a glance: failed runs spend 44.6\% of their steps in \texttt{recover interaction} (retrying, re-searching, re-navigating) versus only 12.0\% for successful runs.
% 对任务导出的 338 个 bad--good pair 构建带符号差分 profile（bad 减 good，Figure~\ref{fig:diff-flamegraph}），一眼给出答案：失败运行花 44.6\% 的步骤在 \texttt{recover interaction}（重试、重搜索、重导航），而成功运行只有 12.0\%。
...
This structure matches independent expert looping labels on 435 unique trajectories at AP .634 versus prevalence .398 (AP-minus-prevalence interval [.181, .293] over 10{,}000 task-cluster draws; computed per unique trajectory after deduplicating pair reuse), confirming the profile surfaces a real behavioral pattern, not an artifact of step count; a fixed-chain repeated/error control reaches the same detection quality (AP .656; recursive-minus-fixed interval [-.107, .061]), while only the recursive profile provides the decomposition above with drilldown to source.
% 该结构在 435 条唯一轨迹上与独立专家 looping 标注的一致性为 AP .634（prevalence .398；10,000 次 task-cluster 抽样的 AP 减 prevalence 区间 [.181,.293]；按去重后的唯一轨迹计算），确认 profile 呈现的是真实行为模式而非步骤数的副产物；fixed-chain repeated/error 对照达到相同检测质量（AP .656；recursive 减 fixed 区间 [-.107,.061]），但只有递归 profile 提供上述可下钻到 source 的分解。
...
% 要点。profile 呈现的失败结构在集合尺度上与独立人工标注一致；profile 在既有 judge 之上补充定位信号；语义名称集中读者注意力，在相同排序质量下将需要打开的证据从 65\% 降至 53\%。
...
Name normalization then maps the 3{,}895 free-form interval names to 783 stable short names (e.g., \texttt{diagnose authentication}) with zero adjacent display-path collisions, failing closed on unresolved ones; identical names across sessions merge so that recurring work folds in the final profile.
% 名称归一化把 3,895 个自由形式的区间名映射为 783 个稳定短名称（如 \texttt{diagnose authentication}），不产生任何相邻显示路径冲突、无法消解时 fail closed；跨 session 的相同名称合并，使重复工作在最终 profile 中折叠。
...
over nine fixed visible fields with five session-held-out folds---parameters
...
AgentRewardBench row is the fixed public release sample used by the cost
harness, distinct from RQ2's 7{,}229-operation mixed population.}
\label{tab:rq4-cost}
\end{table}
...
\emph{Takeaway.} A corpus pays minutes of active backend time once; every
later view, measure switch, and drilldown costs about a second.
% 要点。一个语料只需支付一次分钟级的 active backend 时间；之后每次视图、度量切换与下钻的成本约为一秒。
