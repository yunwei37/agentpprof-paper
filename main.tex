...
Instead of assigning each step a class, the backend reads a trajectory
and finds the transition points where the agent's responsibility changes.
% backend 不为每步指派类别，而是读取轨迹、找出责任变化处的转换点。
...
The sparse marks still produce a complete segmentation: turns between two
marks inherit the preceding path. One format retry is allowed; afterwards,
deterministic prefix repair and canonicalization leave the downstream
pipeline unchanged.
...
The backend works in an annotation workspace where the CLI validates
that marks are nested, noncrossing, and cover every source node (each
source node corresponds to one turn in the trajectory), then emits one
standard pprof protobuf profile that existing tools such as
\texttt{go tool pprof} read directly.
...
The backend reads the trajectory and writes interval annotations that
map source positions to semantic tags and parent links; after validation
\sys updates derived paths and the folded aggregate.
...
Nonblocking hierarchy warnings flag degenerate structures (single-child
recursion, flat fan-outs, coarse leaves), exposing annotation issues
without forcing artificial depth or blocking generation.
...
depth five without a fixed limit.
...
In this local-history scenario, the profiler initializes the annotation
workspace directly from the raw session logs and materializes 10{,}423
source nodes: 42 sessions, 1{,}252 prompts, 5{,}620 LLM calls, and
3{,}509 tool calls carrying 1{,}380{,}863{,}014 provider-reported tokens.
...
The profiling core (parsing, stack construction, folding, and serialization)
is measured by comparing the fixed semantic hierarchy against the raw
hierarchy on identical inputs over three runs across four complete public
workloads and their union, on a 24-core workstation.
...
12{,}050{,}384 input and 231{,}886 output tokens in 37 minutes of
active backend time. After annotation, the deterministic downstream
pipeline completes in under 12 seconds.
...
1.2 seconds at under 500\,MiB peak memory, about 20\% more time than
raw-action grouping. Switching the additive measure is a replay at the
same cost.
