...
Segmentation builds one hierarchy over all three executions (489 operations, 4{,}558{,}192 tokens, semantic depth five with no fixed limit; Figure~\ref{fig:flamegraph}).
...
In evaluation, we use a frontier code agent invoked once per trajectory with one fixed instruction:
% 在评估中，我们使用前沿 code agent，每条轨迹调用一次，使用固定指令：
...
% 输入：一条轨迹（每步的提示词、命令与输出摘要，不含标签或分数）。动作：读一次，在责任变化处发出 mark。输出：稀疏路径 mark，仅在路径变化处出现一行，名称为一到三个动词开头的词。
...
The CLI validates that interval marks are nested and cover every trajectory step, then emits a standard pprof protobuf profile that \texttt{go tool pprof} reads directly.
% CLI 校验区间标记嵌套且覆盖轨迹的每一步，然后输出标准 pprof protobuf profile，供 \texttt{go tool pprof} 直接读取。
...
The focused task contains 489 operations and 4{,}558{,}192 tokens from three agent frameworks: by count Terminus2 contributes 56.24\% and OpenHands 43.76\%, while by tokens OpenHands contributes 86.62\%.
% 该聚焦任务包含来自三个 agent 框架的 489 个 operation 与 4,558,192 个 token：按 count，Terminus2 占 56.24\%、OpenHands 占 43.76\%；按 token，OpenHands 占 86.62\%。
...
The same hierarchy also replays under elapsed time with exact conservation: the subtree holds 1{,}492 of 3{,}982 seconds (37.47\%).
% 同一层次还按 elapsed time 精确守恒地重放：该子树占 3,982 秒中的 1,492 秒（37.47\%）。
Its attributed share therefore rises from 21.47\% by count through 37.47\% by time to 46.15\% by tokens: changing only the measure moves the share by more than a factor of two.
% 因此其归因份额从 count 的 21.47\% 经 time 的 37.47\% 升至 token 的 46.15\%：仅改变度量就使份额变化超过两倍。
The count view attributes the task mainly to Terminus2 while the token view attributes nearly half to the authentication responsibility.
% count 视图把该任务主要归因于 Terminus2，token 视图则把接近一半归因于认证责任。
...
The profiler reads local session logs directly: 42 sessions, 1{,}252 prompts, 5{,}620 LLM calls, and 3{,}509 tool calls carrying 1.38 billion tokens.
% profiler 直接读取本地 session log：42 个 session、1,252 个 prompt、5,620 个 LLM call 与 3,509 个 tool call，携带 13.8 亿 token。
Segmentation produces 1{,}737 annotations at depths two through four, and the operation-count and token profiles conserve exactly 3{,}509 and 1{,}380{,}863{,}014.
% 切分产生 1,737 个深度二至四的标注，operation-count 与 token profile 分别精确守恒 3,509 与 1,380,863,014。
...
The same hierarchy replays under file-read, file-write, and network widths (Figure~\ref{fig:selfprofile-write}); each created file appears as a drilldown leaf beneath the operation that created it.
% 同一层次还按 file-read、file-write 与 network 宽度重放（Figure~\ref{fig:selfprofile-write}）；每个被创建的文件都作为下钻叶出现在创建它的 operation 之下。
