...
% RQ2 问 profiler 输出（构建时不接触真实故障标注）是否对应独立标注的真实问题。我们在三个层次上回答：（1）440 次运行的差分分析，其 recovery 信号对应专家 looping 标注；（2）三个完整 benchmark 上的定位测试；（3）测量人类需要打开多少证据的阅读研究。
...
% 语义前缀在另外两个被测量的角色中有贡献：跨运行归因（RQ1）与引导读者的注意力，后者由下面的研究测量。
...
It reaches MAP .455 while opening 53.0\% of the source content; missed targets were absent from the reader's selection entirely, not cut off by the group budget.
% 它在只打开 53.0\% source 内容的情况下达到 MAP .455；遗漏的 target 完全不在 reader 的选择中，而非被 group budget 截断。
Under the information-equivalent \sys-Raw skeleton, ranking quality is
statistically unchanged (MAP .465), but the reader opens significantly more content (65.0\% versus 53.0\%; Figure~\ref{fig:rq2-deltas}).
% 在信息量等价的 \sys-Raw 骨架下，排序质量在统计上不变（MAP .465），但 reader 打开的内容显著更多（65.0\% 对 53.0\%；Figure~\ref{fig:rq2-deltas}）。
...
% 每 query 的全量阅读还受模型上下文窗口约束——4,558,192-token 的 Git population 无法整体读入——而由骨架引导的阅读在 profile 存在后对任意 trace 长度都可用；此外 reader 是 query-specific 的，而层次只构造一次并跨 query 与度量重放。
...
signal on top of existing judges; and semantic names concentrate a reader's attention, reducing evidence opened from 65\% to 53\% at equal ranking quality.
% 要点。profile 呈现的失败结构在 435 条轨迹上与独立人工标注一致；profile 在既有 judge 之上补充定位信号；语义名称集中读者注意力，在相同排序质量下将需要打开的证据从 65\% 降至 53\%。
