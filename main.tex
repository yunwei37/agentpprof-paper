...
The model supports both offline evaluation (as presented here) and online incremental annotation during agent execution.
...
% 定量对照显示为什么两种替代方式都失败。Raw-action 分组（按字面 action-type）失败：105 个认证 operation 中 102 个只带标签 \texttt{run}，而同一 bucket 还混入 97 个无关 operation；更粗的 action-kind 分组把工作散落到六种 kind。Native call tree（每个 operation 在其源 LLM 或 tool call 下）把每个 operation 放在不同 source call 下，没有共享责任节点。只有语义层次将认证重聚为一条可聚焦的跨运行路径，同时保留所有 call/tool 证据作为下钻叶。
...
On the complete 220 queries that contain a faulty step, a Grok 4.5~\cite{grok45} agent (agent-as-judge) receives the task description and trajectory content (without any ground-truth labels) and is asked to rank operations by likely fault.
% 在全部 220 个含出错步骤的 query 上，Grok 4.5 agent（agent-as-judge）接收任务描述与轨迹内容（不含任何真实标签），被要求按可能出错排序 operation。
Full-trace reading (without \sys) reaches MAP .502 at a mean 12{,}615 input tokens per query, versus .209 for the benchmark's Direct-only diagnostic and .326 for Direct+\sys.
% 全 trace 阅读（不用 \sys）以每 query 平均 12,615 个输入 token 达到 MAP .502，而 benchmark 的 Direct-only 诊断为 .209、Direct+\sys 为 .326。
A profile-guided reader uses the semantic hierarchy to select at most five groups and opens only their evidence.
% profile 引导的 reader 用语义层次选择最多五个 group，并只打开其 evidence。
With semantic names, the reader reaches MAP .455 while opening 53.0\% of the source content; with raw-action names, it opens 65.0\% at unchanged MAP (.465; paired $\Delta=+.010$ [$-.021$,$+.042$]; content $\Delta=+.120$ [$+.103$,$+.137$]).
