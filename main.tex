...
Group scores aggregate frozen benchmark-judge predictions per group: vote averaging for AgentProcessBench, and for HINTBench and TraceElephant the 95\% Wilson lower bound~\cite{wilson1927} of each root-to-frame prefix's positive-prediction fraction, an operation taking the maximum over containing prefixes; a separate 80-trajectory HINTBench validation snapshot selects one of 24 candidate field orders before all 536 tests are scored.
% Group 分数按 group 聚合冻结的 benchmark-judge 预测：AgentProcessBench 用投票平均，HINTBench 与 TraceElephant 用每个 root-to-frame 前缀正预测比例的 95\% Wilson 下界，operation 取其所在前缀的最大值；一个独立的 80 条 HINTBench validation snapshot 在为全部 536 条 test 打分前从 24 种候选字段序中选定一种。
...
retained evidence, not the semantic names, drives this ranking gain.}
...
retained evidence---not naming alone---drives the localization gain over
Direct-only.
...
target-blind TF-IDF/K-Means method reaches V-measure 0.557 on nine Mind2Web
sessions and 0.815 on 100 ScienceWorld sessions~\cite{mind2web,scienceworld}.
...
Constructing the 405 source-only packets takes 501.64\,s median, and after annotation the full deterministic downstream pipeline (assembly, repair, validation, canonicalization, profile construction and readback, scoring) completes in 11.516\,s.
% 构造 405 个 source-only packet 的中位时间为 501.64 秒；标注完成后，完整的确定性下游管线（组装、修复、校验、规范化、profile 构建与读回、打分）在 11.516 秒内完成。
