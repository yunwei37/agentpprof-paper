...
% 切分在三次执行之上构建同一个层次（489 个 operation、4,558,192 个 token，语义深度在无固定上限下达到五层；Figure~\ref{fig:flamegraph}）。
...
annotated real problems. We answer at three levels: a population-scale
differential case whose recovery signal matches expert looping labels, a
supplement test over three complete localization benchmarks, and a
reading-index study measuring how much source evidence a reader must open.
% RQ2 问 target-blind profiler 输出是否对应独立标注的真实问题。我们在三个层次上回答：population 尺度的差分案例（其 recovery 信号对应专家 looping 标注）、三个完整 localization benchmark 上的补充测试，以及测量 reader 需要打开多少 source evidence 的阅读索引研究。
...
marks each workload's best methods, and the two direct-first columns are
...
deterministically and one single-shot call per stage.
...
complete-path marks over 17{,}148 source-native steps without access to
...
requesting a depth.
% 直接 Agent 只读一遍完整的 source-only packet，并在 17,148 个 source-native 步骤上发出稀疏的完整路径 mark，物化之前不接触官方 stage 或 outcome：共 4,496 个 mark，语义深度为一层（3 个）、两层（2,873 个）、三层（1,588 个）与四层（32 个），且没有任何 prompt 指定深度。
...
accuracy (majority baseline 0.044 / 0.248), and a separate classifier
...
% 在闭标签的字面标签上，固定的、仅用于评测的 Qwen3.6-27B 方法把全部 1,012 个 AgentBoard goal 分入九个 task family，macro-F1 与 accuracy 为 0.695 / 0.733（majority 基线 0.044 / 0.248）；另一个分类器把来自 120 条 AutoCodeRover、OpenHands 与 RepairAgent 轨迹的全部 2,737 个公开 action 标签分类，macro-F1 与 accuracy 为 0.498 / 0.628（majority 基线 0.061 / 0.323）。
...
\emph{Results.} Segmenting all 405 CodeTraceBench trajectories consumes 12{,}050{,}384 input and 231{,}886 output tokens in 37 minutes of model time.
% 结果。切分全部 405 条 CodeTraceBench 轨迹消耗 12,050,384 输入与 231,886 输出 token、37 分钟模型时间。
