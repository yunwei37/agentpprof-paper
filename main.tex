...
The evaluation asks whether semantic profiling improves resource attribution (RQ1), whether profiler output corresponds to real problems (RQ2), how accurate the tags are (RQ3), and what the profiling costs (RQ4).
% 评估问：语义 profiling 是否改善资源归因（RQ1），profiler 输出是否对应真实问题（RQ2），标签有多准确（RQ3），以及 profiling 成本多少（RQ4）。
...
A signed difference profile over the 338 bad--good pairs the tasks induce (bad minus good, Figure~\ref{fig:diff-flamegraph}) shows the answer directly: failed runs spend 44.6\% of their steps in \texttt{recover interaction} (retrying, re-searching, re-navigating) versus only 12.0\% for successful runs.
% 对任务导出的 338 个 bad--good pair 构建带符号差分 profile（bad 减 good，Figure~\ref{fig:diff-flamegraph}）直接呈现答案：失败运行花 44.6\% 的步骤在 \texttt{recover interaction}（重试、重搜索、重导航），而成功运行只有 12.0\%。
...
intervals from 10{,}000 trajectory-cluster resamples.
Adding the profile to each benchmark's own judge improves
ranking on all three workloads (left bars, intervals entirely above zero),
whereas the semantic prefix and the raw-action ablation
are indistinguishable (right bars, intervals spanning zero). Grouping plus
retained evidence drives the ranking gain; semantic names improve evidence efficiency in the reading study below.}
...
\emph{Setup.} Gold structure is the 2{,}948 human-annotated stages over
