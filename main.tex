...
RQ2 asks whether profiler output, constructed without seeing ground-truth fault annotations, corresponds to independently annotated real problems.
We answer with three tests: (1) \emph{differential analysis} checks whether the profile's failure structure matches expert behavioral labels across 440 runs; (2) \emph{localization} checks whether adding profile scores to existing benchmark judges improves fault ranking; (3) \emph{reading study} checks whether semantic names help an agent reader reach the same ranking while opening less evidence.
% RQ2 问 profiler 输出（构建时不接触真实故障标注）是否对应独立标注的真实问题。我们用三个测试回答：(1) 差分分析检验 profile 的失败结构是否在 440 次运行上与专家行为标注一致；(2) 定位检验在既有 benchmark judge 上添加 profile 分数是否改善故障排序；(3) 阅读研究检验语义名称是否帮助 agent reader 在打开更少证据的情况下达到同等排序。

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figures/agentreward-recovery-bad-excess.operations.png}\\[1pt]
\includegraphics[width=\linewidth]{figures/agentreward-completion-good-excess.operations.png}
\caption{Signed difference profile (bad minus good) over 338 pairs. \emph{Top:} \texttt{recover interaction} dominates bad runs (3,286 vs 455). \emph{Bottom:} \texttt{report completion} favors good runs (191 vs 135).}
...
% 我们把同任务的每次失败运行与一次成功运行配对（338 对），分别构建 profile 并相减（失败减成功）得到带符号差分 profile（Figure~\ref{fig:diff-flamegraph}）。答案立即呈现：失败运行花 44.6\% 的步骤在 \texttt{recover interaction}（重试、重搜索、重导航），而成功运行只有 12.0\%。
...
\emph{Setup.} We compare segmentation output against 2{,}948 human-annotated stages over all 405 CodeTraceBench trajectories. B$^3$
...
Packet construction bounds every text preview (prompt and response fields to at most 1{,}800 characters, tool commands to 600, and status previews to 200), while additive measures come from untruncated structured counters.
% Packet 构建对所有文本 preview 限长（prompt 与响应字段至多 1,800 字符、工具命令 600 字符、状态 preview 200 字符），而可加度量取自未截断的结构化计数器。
...
\emph{Setup.} Cost separates into a one-time automatic segmentation pass per corpus and deterministic construction/replay afterwards (Table~\ref{tab:rq4-cost}).
