...
(one to three words, like \texttt{diagnose authentication}) reappears across sessions and folds.
...
In evaluation, we use Codex~\cite{codex}, a frontier code agent, invoked once per trajectory with one fixed instruction:
% 在评估中，我们使用 Codex——一种前沿 code agent——每条轨迹调用一次，使用固定指令：
...
---one line only where the path changes; names are one to three words starting with a verb.
\end{quote}
...
We run the complete AgentProcessBench, HINTBench (536 of the paper-reported 629 trajectories), and TraceElephant workloads; all profiler outputs are computed before any fault annotations are revealed.
% 我们在完整的 AgentProcessBench、HINTBench（论文报告的 629 条中的 536 条）与 TraceElephant workload 上运行；所有 profiler 输出在故障标注揭示之前计算完成。
Each query is one trajectory containing an annotated faulty step; we report
average precision (AP) averaged over 614, 400, and 220 such queries. Trajectories without a faulty step (522 total) are excluded from MAP since AP is undefined when no relevant item exists.
% 每个 query 是一条含标注出错步骤的轨迹；我们报告 average precision (AP) 在 614、400 与 220 个这样的 query 上的平均值。无出错步骤的轨迹（共 522 条）从 MAP 中排除，因为没有相关项时 AP 无定义。
...
\bottomrule
\end{tabular}
\caption{MAP over the complete RQ2 benchmarks (614, 400, and 220
queries with annotated faulty steps). \sys-Raw uses raw action types instead of semantic names; bold
...
\caption{Paired MAP differences with 95\% confidence intervals from 10{,}000 bootstrap resamples. Adding the profile (constructed without seeing fault labels) to each benchmark's own diagnostic improves ranking on all three workloads (left bars above zero), whereas the semantic names and raw-action ablation are indistinguishable (right bars span zero). Grouping plus retained evidence, not the names, drives this gain.}
...
On the complete 220 queries that contain a faulty step, a Grok-family model receives the task description and trajectory content (without any ground-truth labels) and is asked to rank operations by likely fault; operations appear in original order with one model call per stage.
% 在全部 220 个含出错步骤的 query 上，Grok 家族模型接收任务描述与轨迹内容（不含任何真实标签），被要求按可能出错排序 operation；operation 按原始顺序呈现，每阶段一次模型调用。
...
A two-stage variant first shows only the profile's group names (no actual trajectory content), and the model selects up to five groups to examine;
stage two then opens only those groups' content.
% 两阶段变体先只展示 profile 的组名称（不含实际轨迹内容），模型选择最多五个组查看；第二阶段只打开这些组的内容。
This reaches MAP .455 while opening only 53.0\% of the trajectory content.
% 这在只打开 53.0\% 轨迹内容的情况下达到 MAP .455。
Under the raw-action ablation (same information, no semantic names), ranking quality is
unchanged (MAP .465), but the model opens significantly more content: 65.0\% versus 53.0\%.
% 在 raw-action 消融（同样信息，无语义名称）下，排序质量不变（MAP .465），但模型打开的内容显著更多：65.0\% 对 53.0\%。
...
A per-query full read is also bounded by the model's context window (the
4.6M-token Git trajectories cannot fit), whereas
profile-guided reading works at any length once the profile exists.
% 每 query 的全量阅读还受模型上下文窗口约束——460 万 token 的 Git 轨迹装不下——而 profile 引导的阅读在 profile 存在后对任意长度都可用。
...
Codex~\cite{codex} reads each trajectory once (prompts, commands, and output summaries, with no labels or scores visible) and emits sparse interval marks over 17{,}148 steps without access to official stage annotations: 4{,}496 marks at semantic depth one (3), two (2{,}873), three (1{,}588), or four (32), with no prompt requesting a specific depth.
% Codex 只读一遍轨迹（提示词、命令与输出摘要，不含标签或分数），在 17,148 个步骤上发出稀疏区间标记，不接触官方 stage 标注：共 4,496 个标记，语义深度为一层（3 个）、两层（2,873 个）、三层（1,588 个）与四层（32 个），且没有任何 prompt 指定深度。
...
Benchmark's own tree & 0.975 & 0.249 & 0.397 & 0.259 \\
One step = one group & 0.983 & 0.221 & 0.361 & 0.246 \\
\bottomrule
\end{tabular}
\caption{Agreement with human stages on all 405 CodeTraceBench
trajectories. Causal Qwen2.5-3B is a stateful per-turn LLM baseline that produces real internal boundaries.}
...
segmenter 0.680 / 0.786 (strongest control: 0.645 / 0.678), and an unsupervised
TF-IDF/K-Means method (no labels seen) reaches V-measure 0.557 on nine Mind2Web sessions and
...
When the same Codex instruction is frozen and transferred to OSWorld-Human without adaptation, it reaches only 0.448 B$^3$ F1 and 0.149 boundary F1, predicting 569 groups for 2{,}042 gold groups---severe under-segmentation that shows cross-population generalization remains a limitation.
% 当同一 Codex instruction 冻结后不经调整迁移到 OSWorld-Human 时，只达到 0.448 B$^3$ F1 与 0.149 边界 F1，对 2,042 个 gold group 预测 569 个——严重欠切分，表明跨 population 泛化仍是局限。
