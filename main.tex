...
% 分析开销是会话结束后花费的时间和资源。
Each profiling run chooses a stack-field list, a predicate, a weight function, and ranking criteria, then executes the full pipeline (parse, enrich, construct stacks, fold) on the input operations.
% 每次 profiling 运行选择栈字段列表、谓词、权重函数和排名标准，然后在输入 operation 上执行完整管线（解析、丰富、构造栈、折叠）。
