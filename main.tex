...
Changing the fields changes the attribution without changing the data: the same operations can be attributed by task, by session, or by action type.
...
Segmentation is iterative: the agent can revise marks incrementally without re-annotating the entire trajectory.
% 切分是迭代的：agent 可以增量修改标记而无需重新标注整条轨迹。
...
Multi-agent orchestration works automatically: subagent operations inherit the outer agent's semantic path.
...
% 已被字面记录的字段（如 skill 调用）无需标注成本即可成为栈层级。多智能体编排自动运作：subagent 操作继承外层 agent 的语义路径。还提供非 LLM 统计 segmenter，用 NPMI 为转移评分并用 $k$-means 校准无监督阈值。
%% ===================================================================
\section{Evaluation}
% 评估
\label{sec:eval}
...
Benchmark trajectories average 8--52 operations (short to medium); the 42-session from the research of this paper portion supplies long-horizon coverage, whose longest sessions span tens of hours, and the RQ4 union covers 27{,}765 operations.
...
Full-trace reading reaches MAP .502 at 12.6K tokens/query; a profile-guided reader using semantic names reaches .455 while opening only 53\% of the source (vs 65\% with raw-action names at unchanged MAP).
