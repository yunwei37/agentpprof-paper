...
It reads Codex and Claude Code local JSONL history files, Agentsight~\cite{agentsight}  trace files and monitor records for system effects, and reconstructs prompts, LLM calls, tool invocations, and their triggered file and network effects.
% \texttt{agent-session} crate 读取 Codex 和 Claude Code 本地 JSONL 历史文件，重建提示词、LLM 调用、工具调用及其触发的文件和网络副作用。
The algorithm can be implemented in any language as a server, e.g. python backend or
