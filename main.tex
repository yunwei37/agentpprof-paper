...
First, the responsible entities are semantic categories such as task intent and workflow phase, not code paths.
Second, agent trajectories lack the stable identifiers and structural hierarchies that make attribution possible.
% 智能体行为与代码执行不同，给 profiling 带来两个挑战。其一，负责实体是语义类别（如任务意图和工作流阶段）而非代码路径。其二，智能体轨迹缺乏使归因成为可能的稳定标识符和结构层次。
...
A quantitative control shows why both alternatives fail: of 105 authentication operations, 102 carry only the raw-action label \texttt{run}, and that same \texttt{run} bucket mixes in 97 unrelated operations; a coarser action-kind grouping (mapping raw actions to semantic categories like execute, version-control, edit) scatters the work across six categories (execute 39\%, version-control 22\%, edit 19\%, inspect 12\%, search 7\%, install 1\%); native call trees place each operation under a distinct source call with no shared responsibility node.
...
% 定量对照显示为什么两种替代方式都失败：105 个认证 operation 中 102 个只带 raw-action 标签 \texttt{run}，而同一个 \texttt{run} bucket 还混入 97 个无关 operation；更粗的 action-kind 分组（将 raw action 映射为 execute、version-control、edit 等语义类别）把工作散落到六个类别（execute 39\%、version-control 22\%、edit 19\%、inspect 12\%、search 7\%、install 1\%）；native call tree 把每个 operation 放在不同 source call 下，没有共享责任节点。只有语义层次将认证重聚为一条可聚焦的跨运行路径，同时保留所有 call/tool 证据作为下钻叶。
...
% 跨 440 条 AgentRewardBench 轨迹（7,229 个 operation、51,904,621 个 token，均精确守恒），按 count 与 token 排序的一致性高：在 125 个任务中至少有三个不同 operation 的 77 个上，平均 Kendall tau-b 为 0.886 [0.857, 0.915]，Spearman rho 为 0.935 [0.917, 0.953]，pooled 排序的 tau-b 为 0.929。然而 77 个任务中有 10 个低于 0.7，因此关键时刻度量选择很重要。
...
This structure matches independent expert looping labels on 435 unique trajectories at AP .634 versus prevalence .398 (difference interval [.181, .293] over 10{,}000 task-cluster draws; computed per unique trajectory after deduplicating pair reuse), confirming the profile surfaces a real behavioral pattern, not an artifact of step count; a fixed-chain repeated/error control reaches the same detection quality (AP .656; difference interval [-.107, .061]), while only the recursive profile provides the decomposition above with drilldown to source.
% 该结构在 435 条唯一轨迹上与独立专家 looping 标注的一致性为 AP .634（prevalence .398；10,000 次 task-cluster 抽样的差异区间 [.181,.293]；按去重后的唯一轨迹计算），确认 profile 呈现的是真实行为模式而非步骤数的副产物；fixed-chain repeated/error 对照达到相同检测质量（AP .656；差异区间 [-.107,.061]），但只有递归 profile 提供上述可下钻到 source 的分解。
...
non-interpolated per-query AP averaged over 614, 400, and 220 labeled
queries; all 522 unlabeled trajectories are consumed for population
coverage but excluded from MAP because AP is undefined without a relevant
item.
% 每个 query 是一条含标注 target 的轨迹，我们报告未插值的 per-query AP，并在 614、400 与 220 个带 target 的 query 上取算术平均；全部 522 条 unlabeled 轨迹计入群体覆盖，但因缺少相关项使 AP 无定义而从 MAP 中排除。
...
\emph{Direct-only} ranks by that diagnostic alone; \emph{Direct+AgentProf} breaks ties using the profiler's aggregated scores per semantic group;
...
% Direct-only 仅按该诊断排序；Direct+AgentProf 用 profiler 的组分数打破并列；Direct+\sys-Raw 是将语义区间名替换为 raw action 类型的消融（分组结构相同，仅名称不同）；AgentProf-only 仅用 profiler 不用 benchmark 诊断（Table~\ref{tab:rq2-localization}）。
...
(Table~\ref{tab:rq2-localization}; right bars of
Figure~\ref{fig:rq2-deltas} span zero), confirming that grouping plus
retained evidence drives the localization gain over Direct-only.
% 语义与 \sys-Raw 两种细化达到相同的排序质量（Table~\ref{tab:rq2-localization}；Figure~\ref{fig:rq2-deltas} 右侧柱区间跨零），确认了"分组+保留的证据"驱动相对 Direct-only 的定位增益。
The semantic prefix contributes in two other measured roles:
cross-run attribution (RQ1) and directing an agent's attention, which the
following study measures.
% 语义前缀在另外两个被测量的角色中有贡献：跨运行归因（RQ1）与引导 agent 的注意力，后者由下面的研究测量。
...
Full-trace reading reaches MAP .502 at a mean 12{,}615 input tokens per query, versus .209 for the benchmark's Direct-only diagnostic and .326 for Direct+AgentProf.
...
skeleton, with no source content, and the agent selects at most five groups;
stage two then opens only those groups' evidence.
% 两阶段的 profile 引导变体先只展示语义 operation 骨架（不含 source 内容），agent 最多选择五个 group；第二阶段只打开这些 group 的证据。
It reaches MAP .455 while opening 53.0\% of the source content; missed targets were absent from the agent's selection entirely, not cut off by the group budget.
% 它在只打开 53.0\% source 内容的情况下达到 MAP .455；遗漏的 target 完全不在 agent 的选择中，而非被 group budget 截断。
Under the information-equivalent \sys-Raw skeleton, ranking quality is
statistically unchanged (MAP .465), but the agent opens significantly more content (65.0\% versus 53.0\%; Figure~\ref{fig:rq2-deltas}).
% 在信息量等价的 \sys-Raw 骨架下，排序质量在统计上不变（MAP .465），但 agent 打开的内容显著更多（65.0\% 对 53.0\%；Figure~\ref{fig:rq2-deltas}）。
Semantic naming's measured contribution in this setting is therefore
attention concentration at equal quality.
% 因此在该设置下，语义命名可测得的贡献是"同等质量下的注意力集中"。
...
reading guided by the skeleton stays available at any trace length once the profile
exists. The agent reader is query-specific while the hierarchy is constructed
once and replayed across queries and measures.
% 每 query 的全量阅读还受模型上下文窗口约束（4,558,192-token 的 Git population 无法整体读入），而由骨架引导的阅读在 profile 存在后对任意 trace 长度都可用；此外 agent reader 是 query-specific 的，而层次只构造一次并跨 query 与度量重放。
\emph{Takeaway.} Failure structure surfaced by the profile matches
independent human labels across 435 trajectories; profiles add localization
signal on top of existing judges; and semantic names concentrate the agent's attention, reducing evidence opened from 65\% to 53\% at equal ranking quality.
...
The interface generalizes across method families: on all 287 OSWorld-Human
sessions (3{,}978 operations, 3{,}691 adjacent pairs, 2{,}042 human groups),
a supervised Bernoulli Naive Bayes boundary predictor~\cite{mccallum-nigam-1998}
over nine fixed visible fields with five-fold cross-validation (sessions held out, parameters
and threshold fitted on training sessions only) reaches 0.739 boundary F1 /
0.816 B$^3$ F1, calibrated recurrence reaches 0.734 / 0.801 using
group annotations the unsupervised default avoids, the no-LLM recurrence
segmenter reaches 0.680 / 0.786 (strongest control: 0.645 / 0.678), and an unsupervised TF-IDF/K-Means method (no labels seen) reaches V-measure 0.557 on nine Mind2Web sessions and 0.815 on 100 ScienceWorld sessions~\cite{mind2web,scienceworld}; the same Codex instruction without adaptation uses coarser task-level boundaries (0.448 B$^3$ F1); matching OSWorld's finer action-group granularity recovers 0.816, showing the framework supports different segmentation scales.
% 该接口在不同方法家族间泛化：在 287 个 OSWorld-Human session 上，监督边界预测器达到 0.739 边界 F1 / 0.816 B$^3$ F1，无 LLM 的 recurrence segmenter 为 0.680 / 0.786（最强对照 0.645 / 0.678）；无监督的 TF-IDF/K-Means 方法（不接触标签）在 9 个 Mind2Web session 上 V-measure 为 0.557，在 100 个 ScienceWorld session 上为 0.815；同一 Codex instruction 不经调整使用更粗的 task-level 边界（0.448 B$^3$ F1）；匹配 OSWorld 更细的 action-group 粒度后恢复到 0.816，表明框架支持不同切分尺度。
...
The action classifier's 0.437 macro-F1 gain over majority has a
full-trajectory 95\% bootstrap interval of [0.380, 0.494]; 39 AutoCodeRover
inputs literally contain the target word \texttt{Locate}, and excluding them
leaves 0.490 macro-F1 and 0.622 accuracy, so the positive comparison is
unchanged.
% 该 action 分类器相对 majority 的 0.437 macro-F1 增量，其整轨迹 95\% bootstrap 区间为 [0.380,0.494]；39 个 AutoCodeRover 输入字面含目标词 \texttt{Locate}，排除后 macro-F1 与 accuracy 仍为 0.490 与 0.622，正向比较不变。
...
\emph{Setup.} Cost separates into a one-time automatic segmentation pass per corpus and deterministic construction/replay afterwards.
% 设置。开销分为每语料一次性的自动切分 pass 与其后的确定性构建/重放。
The profiling core (parsing, stack construction, folding, and serialization) is measured by comparing the fixed semantic hierarchy \texttt{project, agent, task, phase, op, tool, status} against the raw hierarchy \texttt{project, agent, action, status} on identical inputs over three runs across four complete public workloads and their union (Table~\ref{tab:rq4-cost}), using \texttt{agentpprof 0.2.37} on a 24-core Intel Core Ultra 9 285K with 125\,GiB RAM and Linux 6.15.11.
% profiling 核心（解析、栈构建、折叠、序列化）的测量方式是：在四个完整公开 workload 及其并集上（Table~\ref{tab:rq4-cost}）、以完全相同的输入各运行三次，比较固定语义层次 \texttt{project, agent, task, phase, op, tool, status} 与 raw 层次 \texttt{project, agent, action, status}；测量使用 \texttt{agentpprof 0.2.37}，硬件为 24 核 Intel Core Ultra 9 285K、125 GiB 内存、Linux 6.15.11。
...
\label{tab:rq4-cost}
\end{table}
\emph{Results.} Segmenting all 405 CodeTraceBench trajectories with Codex (GPT-5.6-sol-high) consumes 12{,}050{,}384 input and 231{,}886 output tokens in 2{,}215.9\,s (37 minutes) of active backend wall time.
% 结果。用 Codex（GPT-5.6-sol-high）切分全部 405 条 CodeTraceBench 轨迹消耗 12,050,384 输入与 231,886 输出 token，active backend wall time 为 2,215.9 秒（37 分钟）。
Constructing the 405 trajectory inputs (source content only, no outcome labels) takes 501.64\,s median, and after annotation the full deterministic downstream pipeline (assembly, repair, validation, canonicalization, profile construction and readback, scoring) completes in 11.516\,s.
% 构造 405 条轨迹输入（仅源内容，不含 outcome 标签）的中位时间为 501.64 秒；标注完成后，完整的确定性下游管线（组装、修复、校验、规范化、profile 构建与读回、打分）在 11.516 秒内完成。
A full end-to-end segmentation pass on 440 trajectories (with no outcome labels visible to the segmenter) completes all 12 batches in 3{,}521.6\,s wall-clock time (58.7 minutes, summed worker time 6{,}661.7\,s) on two parallel workers, consuming 12{,}039{,}417 actual input tokens (10{,}929{,}408 reported cached, 90.8\%) and 312{,}433 output tokens, or 27{,}362 input and 710 output tokens per trajectory. One fresh pass over the three-trajectory Git population takes 466.9\,s and 832{,}544 actual input tokens.
% 对 440 个 session 的完整端到端切分（切分器看不到 outcome 标签）在双并行 worker 上以 3,521.6 秒 wall-clock 时间（58.7 分钟，worker 时间合计 6,661.7 秒）完成全部 12 个 batch，消耗 12,039,417 个实际输入 token（其中报告缓存 10,929,408 个，占 90.8\%）与 312,433 个输出 token，即每 session 27,362 输入与 710 输出 token。对三 session 的 Git population，一次全新完整 pass 需 466.9 秒与 832,544 个实际输入 token。
...
% 切换可加度量即以同等成本重放。
Deterministic materialization of the full 440-trajectory population takes
...
% \sys 提出的则是 profiler 的问题，即某个语义类别对某种可加资源负有多少责任。它从 source 内容恢复可变深度责任（Act$\cdot$onomy 固定三层、TraceProbe 仅一层）、跨 count/token/time 宽度精确守恒、保留 LLM/tool 下钻叶、输出标准 pprof。没有被比较系统同时具备这四点，且 \sys 与单运行诊断互补，RQ2 直接测量了这一点。
