...
The population contains 41 real long-horizon agent trajectories, 3{,}146 user turns, and 5{,}750 operations, including three independent executions of one Git-deployment task whose 735 steps receive 96 recursive annotations.
% population 含 41 条真实 long-horizon agent 轨迹、3,146 个用户 turn 与 5,750 个 operation，其中包含同一 Git 部署任务的三次独立执行——其 735 个步骤获得 96 个递归标注。
...
RQ2 asks whether profiler output, constructed without seeing the ground-truth fault annotations, corresponds to independently annotated real problems. We answer at three levels: a differential analysis over 440 runs whose recovery signal matches expert looping labels, a localization test on three complete benchmarks, and a reading study measuring how much evidence an analyzer agent must open.
% RQ2 问 profiler 输出（构建时不接触真实故障标注）是否对应独立标注的真实问题。我们在三个层次上回答：（1）440 次运行的差分分析，其 recovery 信号对应专家 looping 标注；（2）三个完整 benchmark 上的定位测试；（3）测量分析器 agent 需要打开多少证据的阅读研究。
...
\emph{Takeaway.} Segmentation costs minutes of backend time once per corpus; every
later view, measure switch, and drilldown costs about a second.
% 要点。切分每语料只需一次分钟级的后端时间；之后每次视图、度量切换与下钻的成本约为一秒。
