...
The resulting hierarchy reaches semantic depth five without a fixed limit, with no unary or flat-fan-out warnings and 27 advisory coarse-leaf warnings.
% 所得层次在无固定上限下达到语义深度五，无 unary 或 flat-fan-out warning，另有 27 条提示性 coarse-leaf warning。
...
Across 440 trajectories, replaying the frozen AgentRewardBench hierarchy (7{,}229 operations and 51{,}904{,}621 tokens, both conserved exactly) ranks the same operations once by count and once by tokens: over the 77 of 125 tasks with at least three distinct operations, mean per-task Kendall's tau-b~\cite{kendall1938} is 0.886 [0.857, 0.915] and Spearman's rho~\cite{spearman1904} is 0.935 [0.917, 0.953], the pooled ranking agrees at tau-b 0.929, and 10 of 77 tasks fall below 0.7---stable in most cases but changing decisions in 10 of 77 tasks.
% 跨 440 条轨迹，重放冻结的 AgentRewardBench 层次（7,229 个 operation 与 51,904,621 个 token，均精确守恒），对相同 operation 分别按 count 与 token 排序：在 125 个任务中至少有三个不同 operation 的 77 个上，每任务平均 Kendall tau-b 为 0.886 [0.857,0.915]，Spearman rho 为 0.935 [0.917,0.953]，pooled 排序的 tau-b 为 0.929——大多数情形稳定，但 77 个任务中有 10 个改变决策。
...
\emph{Direct+\sys-Raw} is an ablation that groups by raw action type instead of semantic names; \emph{AgentProf-only} uses only the profiler without the benchmark diagnostic (Table~\ref{tab:rq2-localization}).
% Direct-only 仅按该诊断排序；Direct+AgentProf 用 profiler 的组分数打破并列；Direct+\sys-Raw 是按 raw action 类型而非语义名分组的消融；AgentProf-only 仅用 profiler 不用 benchmark 诊断（Table~\ref{tab:rq2-localization}）。
...
segmenter reaches 0.680 / 0.786 (strongest control: 0.645 / 0.678), and a
an unsupervised TF-IDF/K-Means method (no labels seen) reaches V-measure 0.557 on nine Mind2Web sessions and 0.815 on 100 ScienceWorld sessions~\cite{mind2web,scienceworld}.
% 该接口在不同方法家族间泛化：在 287 个 OSWorld-Human session 上，监督边界预测器达到 0.739 边界 F1 / 0.816 B$^3$ F1，无 LLM 的 recurrence segmenter 为 0.680 / 0.786（最强对照 0.645 / 0.678）；无监督的 TF-IDF/K-Means 方法（不接触标签）在 9 个 Mind2Web session 上 V-measure 为 0.557，在 100 个 ScienceWorld session 上为 0.815。
When the same instruction is frozen and transferred to OSWorld-Human via Codex with GPT-5.6-sol-high without adaptation, it reaches only 0.448 B$^3$ F1 and 0.149 boundary F1, predicting 569 groups for 2{,}042 gold groups---severe under-segmentation that shows cross-population generalization remains a limitation.
% 当同一 instruction 冻结后通过 Codex（GPT-5.6-sol-high）不经调整迁移到 OSWorld-Human 时，只达到 0.448 B$^3$ F1 与 0.149 边界 F1，对 2,042 个 gold group 预测 569 个——严重欠切分，表明跨 population 泛化仍是局限。
