...
We compare seven grouping methods and rank groups by the fraction of hidden positives they contain.
Flat summary places all operations in one group.
Per-session grouping groups by session boundary.
Native hierarchy uses the dataset's own structure (e.g., episode and step levels).
Raw-action grouping groups by the unprocessed action-type field.
Operation-stack profiling applies the model from \S\ref{sec:design}.
Two oracle baselines add the hidden annotation fields as upper bounds.
% 我们比较七种分组策略，按组包含的隐藏正例比例排序。扁平摘要将所有 operation 放入一个组。固定会话分组按会话边界分组。原生层次使用数据集自身结构（如 episode 和 step 层级）。原始操作分组按未处理的操作类型字段分组。Operation stack 分析应用 \S\ref{sec:design} 的模型。两个 oracle 基线将隐藏标注字段作为上界加入。
...
Hidden = 1 means the method uses hidden annotations (upper bound).
...
These three methods (which do not use hidden annotations) illustrate a tradeoff between inspection effort and coverage.
...
% 三种不使用隐藏标注的分组方法体现了检查工作量与覆盖率之间的权衡。扁平摘要需要检查全部但能找到一切。固定会话分组最小化找到首个正例的工作量，但将其余正例分散到数百个组中。Operation stack 分析平衡两者：它将正例集中到更少、更高质量的组中，以牺牲快速首次命中换取更低总工作量下的更广覆盖。
...
Sweeping the induced-stack depth cap from 1 to 5 shows that AP peaks at depth 3 (median 0.287), while different tasks achieve their best at different depths (2--5), so depth should be tuned per objective.
% 栈深度影响定位质量。将诱导栈深度上限从 1 扫到 5 显示 AP 在深度 3 达到峰值（中位 0.287），不同任务在不同深度（2--5）达到最佳，因此深度应按目标调整。
Modifying stack fields, mapping rules, or ranking criteria and re-running on the same operations improves AP on 5/6 tasks (median improvement 0.038), confirming that the profiler exposes useful tuning axes.
