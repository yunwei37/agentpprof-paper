...
\caption{Semantic flame graphs of real agent trajectories from one
multi-month development project, under three views. \emph{Top (tokens):} width represents token count.
...
fold together in Figure~\ref{fig:rq1-flamegraph}.
% 具体地，在一次 Git 部署执行中，backend 先把 session 切为 \texttt{build deployment system}，再在"智能体从编写配置转向调试访问"处继续切分，得到 \texttt{deploy branches} 与 \texttt{diagnose authentication}；后者内部的 turn 携带路径 \texttt{build deployment system > diagnose authentication}。由于同样两个名称在另外两次执行中再次出现，其开销在 Figure~\ref{fig:rq1-flamegraph} 中折叠到一起。
...
\paragraph{Use case 1: why does the same deployment keep failing?}
% 用例 1：同一个部署为什么反复失败？
An engineer has three independent agent attempts at one real
Git-deployment task, none of which delivered the requested
password-authenticated endpoint, and asks where the effort went and
which shared responsibility explains the repeated failure.
% 一位工程师手上有同一真实 Git 部署任务的三次独立 agent 尝试，均未交付所要求的密码认证端点；他想知道精力花在了哪里、哪个共享责任解释了反复的失败。
...
% 切分 backend 在三次执行之上构建同一个层次（489 个 operation、4,558,192 个 provider-reported token，语义深度在无固定上限下达到五层；Figure~\ref{fig:rq1-flamegraph}）。
The profile answers directly: the repeated \texttt{diagnose
authentication} subtree is 21.47\% of the task by operation count but
46.15\% by tokens, and drilldown shows all three executions verifying
substitute transport paths without ever establishing the
endpoint---nearly half the budget went to one never-successful
diagnosis that a count view understates by more than half.
% profile 直接给出答案：重复出现的 \texttt{diagnose authentication} 子树按 operation 数只占任务的 21.47\%，按 token 却占 46.15\%；下钻显示三次执行都在验证替代传输路径、却从未建立目标端点——近一半预算花在一个从未成功的诊断上，而 count 视图把它低估了一半以上。
A same-input control replays identical operations and weights under
native, coarse-action, and semantic organizations: the responsibility
scatters across 105 native calls and six coarse action kinds (largest
branch 39.42\% of its tokens); only the semantic organization reunites
it into one focusable cross-run path.
...
with exact conservation; it is the only tested organization that
reunites a cross-run responsibility, and the minority of tasks where
count and token rankings diverge is precisely where choosing the measure
changes the engineering decision.
...
glance: recovery work occupies 44.6\% of failed-side versus 12.0\% of
successful-side occurrences (completion 1.8\% versus 5.1\%), and
